...
get_size <- function(board) {
	dim(board)[1];
};

get_midpoint <- function(board) {
	get_size(board) %/% 2 + 1;
};

embiggen <- function (board) {
	size <- get_size(board);
	new_size <- size + 2 * board_size_incr;
	
	bigger_board <- matrix( 0, nrow = new_size, ncol = new_size );
	
	left <- board_size_incr + 1;
	right <- board_size_incr + size;
	bigger_board[left:right,left:right] <- board;
	
	bigger_board;
};

draw_wire_section <- function (
	board,
	wire_id,
	start_x, start_y,
	direction, section_length
) {
	# Make board big enough for new section
	size <- get_size(board);
	while (
		size <= start_x + section_length
		| size <= start_y + section_length
		| start_x - section_length <= 0
		| start_y - section_length <= 0
		) {
		board <- embiggen(board);
		start_x <- start_x + board_size_incr;
		start_y <- start_y + board_size_incr;
		size <- get_size(board);
	}
	
	end_x <- start_x + switch(direction,
														"U" = 0, "D" = 0,
														"L" = -section_length, "R" = section_length);
	end_y <- start_y + switch(direction,
														"U" = section_length, "D" = -section_length,
														"L" = 0, "R" = 0);
	board[start_x:end_x,start_y:end_y] <- bitwOr(
		board[start_x:end_x,start_y:end_y],
		wire_id);
	
	list(board, end_x, end_y);
};

draw_wire <- function (board, wire_id, instructions) {
	instructions <-strsplit(instructions, ",")[[1]];
	
	midpoint <- get_midpoint(board);
	start_x <- midpoint;
	start_y <- midpoint;
	
	nb_instruction <- 1;
	total_instructions <- length(instructions);
	for (instruction in instructions) {
		print(sprintf("Wire %d, section %d/%d", wire_id, nb_instruction, total_instructions));
		nb_instruction <- nb_instruction + 1;
		
		direction <- substr(instruction, 1, 1);
		section_length <- strtoi(substr(instruction, 2, nchar(instruction)));
		
		result <- draw_wire_section(
			board,
			wire_id = wire_id,
			start_x = start_x,
			start_y = start_y,
...
draw_all <- function (wires) {
	board <- matrix( 0, nrow = 3, ncol = 3 );
	
	for (index in 1:length(wires)) {
		wire_id <- 2**(index - 1);
		board <- draw_wire(board, wire_id, wires[index]);
	}
...
find_crossing <- function (board, wires) {
	midpoint <- get_midpoint(board);
	
	all_wires <- 2**length(wires) - 1;
	crossings <- which(board == all_wires, TRUE);
	
	from_midpoint <- abs(crossings - midpoint);
	distances <- from_midpoint[,1] + from_midpoint[,2];
	
	min(distances[distances > 0]);
}

solve <- function(input) {
	wires <- strsplit(input, '\n')[[1]];
	board <- draw_all(wires);
	distance <- find_crossing(board, wires);
	print(input);
	print(distance);
	distance;
}

input_tiny <- "R8,U5,L5,D3
U7,R6,D4,L4";
solve(input_tiny);

input_test_1 <- "R75,D30,R83,U83,L12,D49,R71,U7,L72
U62,R66,U55,R34,D71,R55,D58,R83";
solve(input_test_1);

input_test_2 <- "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51
